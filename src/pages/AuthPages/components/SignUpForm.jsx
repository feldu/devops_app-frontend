...
export default function SignInForm() {
    ...
    const rolesOptions = [{value: "ROLE_MODERATOR", label: "Модератор"}, {value: "ROLE_USER", label: "Пользователь"}];

    if (authInfo.message !== null && authInfo.isError === false) navigate("/auth/signin");

    const signUp = async e => {
        e.preventDefault();
        let {isValid, message} = validateSignUpInput({username, password, name, role});
        setError(!isValid);
        if (!isValid) {
            setErrorMsg(message);
            return;
        }
        dispatch(thunks.registerUser({username, password, name, role}));
    };
                ...
                <Box my={4} textAlign="left">
                    <form>
                        ...
                        <InputPassword value={password} setValue={setPassword} placeholder="От 4 до 20 символов"
                                       label="Пароль"/>
                        <InputText value={name} setValue={setName} placeholder="Не больше 64 символов"
                                   label="Имя"/>
